...
import { Inbox, Trash2, ChevronRight, History } from "lucide-react";
...
export default function TestsPage() {
  ...
  const isLoading = loadingGroups || loadingQuizzes || (!!firebaseUid && loadingAttempts);
  ...
  // Compute recent quizzes (top 10 unique attempted)
  const recentQuizIds = Array.from(
    new Set(
      (attempts as Attempt[])
        .sort((a, b) => {
  ...
  const recentQuizzes = recentQuizIds
    ...
    <main className="min-h-dvh pb-24 pt-16">
      ...
      <div className="px-5 max-w-7xl mx-auto mt-4">
        ...
        <div className="space-y-10">
          {/* Recent Tests Group */}
          {(!firebaseUid || recentQuizzes.length > 0) && (
            <motion.section 
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="w-full"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-white tracking-wide">Recents</h2>
                  {firebaseUid && recentQuizzes.length > 0 && (
                ...
                {firebaseUid && recentQuizzes.length > 0 && (
                  <button 
                    ...
                    className="text-xs font-bold text-purple-400 hover:text-purple-300 transition-colors flex items-center gap-1 uppercase tracking-wider"
                  >
              ...
              {!firebaseUid ? (
                <div className="glass-md rounded-2xl p-6 text-center border border-white/5 bg-white/5 backdrop-blur-sm">
                  <p className="text-gray-400 text-sm mb-3 font-medium">Please login to track your test history</p>
                  <button 
                    onClick={() => router.push('/signup')}
                    className="text-purple-400 hover:text-purple-300 font-bold text-sm underline underline-offset-4 transition-all"
                  >
                ...
                <div className="glass-md rounded-2xl p-8 text-center border border-white/5 bg-white/5">
                  <History className="w-8 h-8 text-gray-600 mx-auto mb-2 opacity-50" />
                  <p className="text-gray-500 text-sm font-medium">Your recent tests will appear here</p>
                </div>
              )}
            </motion.section>
          )}
          ...
          {/* Saved Tests Group */}
          {(!firebaseUid || savedGroupQuizzes.length > 0) && (
            <motion.section 
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="w-full"
            >
              <div className="flex items-center justify-between mb-4">
                ...
                {firebaseUid && savedGroupQuizzes.length > 0 && (
                  <button 
                    onClick={() => router.push('/saved-tests')}
                    className="text-xs font-bold text-purple-400 hover:text-purple-300 transition-colors flex items-center gap-1 uppercase tracking-wider"
                  >
              ...
              {!firebaseUid ? (
                <div className="glass-md rounded-2xl p-6 text-center border border-white/5 bg-white/5 backdrop-blur-sm">
                  <p className="text-gray-400 text-sm mb-3 font-medium">Login to access your bookmarked tests</p>
                  <button 
                    onClick={() => router.push('/signup')}
                    className="text-purple-400 hover:text-purple-300 font-bold text-sm underline underline-offset-4 transition-all"
                  >
                ...
                <div className="glass-md rounded-2xl p-8 text-center border border-white/5 bg-white/5">
                  <Inbox className="w-8 h-8 text-gray-600 mx-auto mb-2 opacity-50" />
                  <p className="text-gray-500 text-sm font-medium">Save tests to practice them later</p>
                </div>
              )}
            </motion.section>
          )}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button 
                    onClick={() => {
                      const slug = group.title.toLowerCase().trim().replace(/\s+/g, '-').replace(/[^\w-]+/g, '');
          ...
          {publishedGroups.length === 0 && savedGroupQuizzes.length === 0 && recentQuizzes.length === 0 && (
